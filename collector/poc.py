...
class POCData:
    ...
    def _load_data_from_csv(self, csv_path: str = ""):
        ...
        self._csvdata[["Day", "Heure"]] = self._csvdata["bin_utc_time_formatted"].str.split(" ", expand=True)
        self._csvdata["Time"] = (
            pd.to_datetime(self._csvdata["Heure"]).diff().fillna(pd.Timedelta(milliseconds=50))
